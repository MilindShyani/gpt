...
class DecoderLayer(nn.Module):
    ...
    def attention(self,x,attn_mask):
        ...
        if self.train:
            ...
            q = self.Wq(x) 
            q = einops.rearrange(q,"B L (n h)->B n L h",h=self.head_dim, L = 1) 
...
class GPT(nn.Module):
    ...
    def generate(self,s):
        self.train = 0
        # Several things that we need to do. The first is to make a forward pass the very first time and store the K,V values        
        out = self.forward(s)
